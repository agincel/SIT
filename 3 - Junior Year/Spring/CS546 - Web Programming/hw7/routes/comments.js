...
router.get('/recipe/:recipeID', function(req, res) {
    recipeData.getRecipe(req.params.recipeID).then((foundRecipe) => {
        res.json(foundRecipe.comments);
    }, () => {
...
router.get('/:commentID', function(req, res) {
    recipeData.getAllRecipes().then((recipeList) => {
        ...
        for(var i = 0; i < recipeList.length && !hasFoundComment; i++) {
            for (var j = 0; j < recipeList[i].comments.length && !hasFoundComment; j++) {
                if (recipeList[i].comments[j]._id == req.params.commentID) {
                    res.json(recipeList[i].comments[j]);
        ...
        res.sendStatus(500).json({error: "Oops"}); //something went wrong server-side
    });
});
